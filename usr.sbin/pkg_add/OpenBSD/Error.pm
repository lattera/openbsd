# ex:ts=8 sw=4:
# $OpenBSD: src/usr.sbin/pkg_add/OpenBSD/Error.pm,v 1.9 2004/12/12 11:26:16 espie Exp $
#
...
sub dienow
{
	...
	if ($error) {
		if ($error =~ m/^(Expected:\s+)?(.*?)(?:\s+at\s+(.*)\s+line\s+(\d+)\.?)?$/) {
			local $_ = $2;
			$OpenBSD: src/usr.sbin/pkg_add/OpenBSD/Error.pm,v 1.9 2004/12/12 11:26:16 espie Exp $3;
			$OpenBSD: src/usr.sbin/pkg_add/OpenBSD/Error.pm,v 1.9 2004/12/12 11:26:16 espie Exp $4;
			$OpenBSD: src/usr.sbin/pkg_add/OpenBSD/Error.pm,v 1.9 2004/12/12 11:26:16 espie Exp $error;
